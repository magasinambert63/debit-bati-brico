    ...
    <div className="app">
      ...
      <div className="layout">
        ...
        <main className="main">
          ...
          <section className="cutlist">
            <h2>Ordre de coupe atelier</h2>
            <table>
              <thead><tr><th>Plaque</th><th>Bande</th><th>Type</th><th>Position</th><th>Dimension bande</th></tr></thead>
              <tbody>
                {panels.flatMap((panel, pIdx) => panel.bands.map((b, bIdx) => (
                  <tr key={`sequence-${pIdx}-${bIdx}`}>
                    <td>{pIdx + 1}</td>
                    <td>{bIdx + 1}</td>
                    <td>{b.type === 'horizontal' ? 'Bande horizontale' : 'Bande verticale'}</td>
                    <td>{b.type === 'horizontal' ? `Y ${b.y.toFixed(1)} cm` : `X ${b.x.toFixed(1)} cm`}</td>
                    <td>{b.w.toFixed(1)} x {b.h.toFixed(1)} cm</td>
                  </tr>
                )))}
              </tbody>
            </table>
          ...
          <section className="labels-sheet">
            <h2>Étiquettes pièces imprimables</h2>
            <div className="labels-grid">
              {items.flatMap(item => Array.from({ length: Number(item.qty || 0) }, (_, idx) => (
                <div className="piece-label" key={`${item.id}-label-${idx}`}>
                  <h3>BATI BRICO</h3>
                  <p><b>Commande :</b> {orderNumber}</p>
                  <p><b>Client :</b> {customerName}</p>
                  <p><b>Pièce :</b> {item.label} {idx + 1}/{item.qty}</p>
                  <p><b>Cote :</b> {item.w} x {item.h} cm</p>
                  <p><b>Matière :</b> {material.product} {material.thickness}</p>
                </div>
              )))}
            </div>
          </section>
          ...
          <section className="checklist">
            <h2>Contrôle avant débit</h2>
            <div className="checks">
              <label><span></span> Cotes client vérifiées</label>
              <label><span></span> Sens du fil / décor validé</label>
              <label><span></span> Panneau contrôlé avant coupe</label>
              <label><span></span> Chutes exploitables mises de côté</label>
            </div>
          </section>
